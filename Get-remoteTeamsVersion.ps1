...
.NOTES
  Version				: 1.2
  Author				: v1.2 - Ben Meyer
								---Added functionality to check a PC remotely.
						  v1.1 - David Paulino 
  Info                  : https://uclobby.com/2018/08/23/teams-check-client-version-using-powershell
  

#>
New-Variable -name "RegPath" -Option Constant -value "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ProfileList"
...
$Profiles = Invoke-Command -computer $hostname {(Get-childItem $using:RegPath  | % {Get-ItemProperty $_.pspath } | ?{$_.fullprofile -eq 1} )}
